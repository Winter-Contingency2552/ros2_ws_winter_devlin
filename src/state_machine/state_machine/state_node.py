    ...
    def control_loop(self):
        ...
        if self.state == 'Initial':
            ...
            if distance_to_goal <= self.pos_tol:
                ...
                self.checkpoint_y = y
                self.checkpoint_yaw = yaw
                self.state = 'SpinCheck'
                self.get_logger().info('Reached Corner Check Point at ({:.2f}, {:.2f}). Preparing to Spin.'.format(goal_x, goal_y))
                self.get_logger().info('State changed to SpinCheck')
            else:
                cmd.linear.x = pid_navigate_cmd.linear.x
                cmd.angular.z = pid_navigate_cmd.angular.z

        elif self.state == 'SpinCheck':
            self.accumulated_angle += self.angular_speed * 0.05
            if self.accumulated_angle >= 2 * math.pi:
                ...
                self.get_logger().info('Spin Check Complete. State changed to OrientToGoal')
            else:
                cmd.linear.x = 0.0
